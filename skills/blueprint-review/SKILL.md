...
13. **Implementation leakage** — Content that belongs in code: function bodies, full SQL statements, rendered config, pinned versions, internal-only log text, test bodies, directory trees. Signatures, type definitions, schema, and externally-visible text stay.
...
5. Note that the reviewed Blueprint is ready as the primary input to `plan-from-context` or `plan-from-prompt` when the user is ready to plan implementation.
